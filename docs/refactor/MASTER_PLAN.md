...
ADR-015 / ADR-016）。**第二次路线修订（作品集级工程要求 + 重建当时的 Phase 11 之后路线）已于
...
**2026-09-23 行政收尾（Phase 7 → Completed / Approved）激活了 Phase 7 的出口决策：
原 Phase 8 被拆分为 `Phase 8 = Migration Chain Repair & Reproducible Baseline` 与
`Phase 9 = Vocabulary Books Implementation`，`Phase 10 = Themed Packs Convergence`；
原 Phase 10–15 的**既有意图、出口条件、安全要求与评估义务全部保留**，
编号整体 +1 顺延为 Phase 11–16**（见本文件末尾的同名记录与 `DECISIONS.md` ADR-022）。
本节中的 Phase 13–16 与加强后的 Phase 11 / 12 出口现为**已批准的权威路线图**。
优先级为：**Vocabulary 平台工作在前，AI Coach 的 Agentic 工作排在后面的独立阶段**。
...
| 8 | Migration Chain Repair & Reproducible Baseline | 修复损坏的 Prisma 迁移链并建立可复现基线；**只做交付 / 迁移治理，不含产品功能** |
| 9 | Vocabulary Books Implementation | 落地已批准的 Book 模型（稳定 `entryKey`）、导入管线与选书体验；**词包保持现状** |
| 10 | Themed Packs Convergence | Default Packs 与 User-created Packs 归位到已批准架构（含跨域 `Word` 收敛与遗留语义移除） |
| 11 | Reliability, Ownership & Evaluation Platform Convergence | 补齐归属、测试、CI、安全边界，并建立评估 / 实验基础设施 |
| 12 | AI Coach Foundation Refactor | 先把既有 AI Coach 重构到已批准架构，并建立确定性 Coach 评估基线 |
| 13 | Retrieval & Knowledge Engineering | 设计、实现并**评估**知识检索层；**同阶段**建立检索最小安全边界，用数据决定 RAG 是否进入生产 |
| 14 | Agentic AI Coach & Tool System | 在**真实动态决策需求**与确定性基线上引入有界 Agent；**同阶段**建立最小 Agent / tool 安全边界 |
| 15 | MCP Interoperability, AgentOps & Safety | 以真实互操作边界落地 MCP，做 MCP 专属与生产风格硬化（**不是**安全的首次建立点） |
| 16 | Production, Benchmark & Portfolio Hardening | 部署、基准 / 实验汇总、作品集与面试材料 |
...
2. **AI Coach 可能成为有界的 Agentic 子系统，但刻意延后。** Agentic 工作排在独立的后续
   阶段（Phase 14），且必须由真实产品行为需求驱动；它之前先经过 Phase 12（基础重构）与
    Phase 13（检索工程评估）。
...
### Phase 7 出口与 Phase 8 范围决策门（2026-09-16 复审修正 R-01 — **已于 2026-09-23 关闭并激活**）

> **结果（2026-09-23）。** Phase 7 的出口决策已作出并激活：**迁移链修复与 Vocabulary Books 实现
> 属于可独立审核的高风险变更，因此必须拆分**。原 Phase 8 已被拆分与顺延为
> **Phase 8 = Migration Chain Repair & Reproducible Baseline**、
> **Phase 9 = Vocabulary Books Implementation**、**Phase 10 = Themed Packs Convergence**，
> 原 Phase 10–15 顺延为 Phase 11–16（见 `DECISIONS.md` ADR-022 与 `VOCABULARY_MIGRATION_STRATEGY.md` §7）。
> 下方原文保留为**历史决策门记录**。
...
配套的迁移正确性要求见 `EVALUATION_BASELINE.md` 的迁移验证门（修复 / 变更迁移的那个已批准 Phase
—— 当前即 **Phase 8** —— 必须**自行**通过迁移正确性验收，不得把首次验证推迟到 Phase 16）。

### Phase 11 范围 / 拆分决策门（2026-09-20 复审修正 R-03；原 Phase 10）
...
**不在本次修订中机械拆分该阶段。** 取而代之，Phase 11（原 Phase 10）实现开始**之前**必须有一个显式的
范围 / 拆分决策门：
...
该决策由 **Phase 11 的已批准任务书 / 外部评审**产出（不预先新增 Phase 编号，也不在本修订内
夹带实现）；若判定应拆分，则在 Phase 11 实现开始前走正常治理流程后拆分。
...
4. **Phase 11 与 Phase 12 的出口被加强**（原 Phase 10 / 11）。Phase 11 从“可靠性收敛”扩展为
   **Reliability, Ownership & Evaluation Platform Convergence**，负责建立后续 AI 实验可测量的
   评估 / 实验基础设施；Phase 12 必须在结束时产出**确定性 Coach 评估基线**，供后续检索与
   Agent 阶段对比。
5. **Phase 13–16 是新增 / 重排的独立可审核阶段**（原 Phase 12–15）。
   Phase 13（Retrieval & Knowledge Engineering）、Phase 14（Agentic AI Coach & Tool System）、
   Phase 15（MCP Interoperability, AgentOps & Safety）各自独立审核；旧的终止阶段被加强为
   Phase 16（Production, Benchmark &
   Portfolio Hardening）。
6. **Phase 0–11 的既有意图被保留。** Phase 7–10 的产品序列与 Phase 12 的
   “先重构 Coach、再 Agent 化 Coach”原则不变；本次修订只加强 Phase 11 / Phase 12 的出口条件，
   并重建 Phase 12 之后的路线。
7. **fine-tuning 门槛。** fine-tuning 不是强制 Phase 或强制技术；只有当后续评估暴露出
   ...
   specialization 实验。
8. **Phase 数量从属于范围。** 见上文同名原则与核心原则 7 / 9；新增 Phase 13–16（原 12–15）是有界范围与
   可审核性的结果，而不是为了凑阶段数。
9. **安全在引入风险的 Phase 内建立最小边界。** 不变量：**引入新风险的 Phase 必须在该 Phase 内
   建立该风险的最小安全边界**；后续 Phase 只能硬化或生产化，**不得**成为安全边界的首次建立点。
   因此 Phase 13 承担检索最小安全（不可信检索内容、provenance、间接注入边界、恶意内容测试、
   证据与系统指令隔离）；Phase 14 承担最小 Agent / tool 安全（allowlist 工具、用户隔离、
   权限边界、读写区分、高风险写入的 confirmation / HITL、预算 / 循环上限、工具失败隔离、
   足以重建关键工具决策的审计 / trace 钩子）；Phase 15 只做 MCP 专属与生产风格硬化。
10. **Phase 14 的 Agent 入口门。** Agent 只有在存在**合法的模型驱动动态决策问题**时才可实现：
...
11. **Phase 11 的范围 / 拆分决策门。** 见上一小节；加强后的 Phase 11（原 Phase 10）可能过大，必须在实现
    开始前显式判定两条工作流能否安全留在同一个有界 Phase。

### Phase 7 关闭与阶段拆分激活（2026-09-23 行政收尾 — 已生效）

这是一次**文档 / 治理层面的行政收尾（administrative closeout）**，不是新的产品功能，也不是 Phase 8 实现：

1. **Phase 7 关闭。** `Phase 7 — Vocabulary Platform Design & Data Provenance` 于
   **2026-09-23 经外部评审 v5 Approved（Blocking Issues: None）**，状态转为 **Completed / Approved**；
   ADR-019…ADR-022 由本次收尾命令从 `Proposed` 转为 **`Accepted`**（含义以 **v5** 设计为准）。
2. **拆分激活（ADR-022 生效）。** 原 Phase 8 拆分为两个独立可审核阶段并顺延：
   - **Phase 8** `Migration Chain Repair & Reproducible Baseline` → **Ready / Not Started**
     （只做迁移链修复与可复现基线；**不含**产品功能）；
   - **Phase 9** `Vocabulary Books Implementation` → **Not Started**；
   - **Phase 10** `Themed Packs Convergence` → **Not Started**。
3. **原 Phase 10–15 顺延 +1，意图与出口条件全部保留：**

   | 旧编号 | 名称 | 新编号 |
   |--------|------|--------|
   | 10 | Reliability, Ownership & Evaluation Platform Convergence | **11** |
   | 11 | AI Coach Foundation Refactor | **12** |
   | 12 | Retrieval & Knowledge Engineering | **13** |
   | 13 | Agentic AI Coach & Tool System | **14** |
   | 14 | MCP Interoperability, AgentOps & Safety | **15** |
   | 15 | Production, Benchmark & Portfolio Hardening | **16** |

4. **历史不被改写。** Phase 0–7 的已批准含义、编号与历史记录保持不变；
   历史文档中出现的旧编号（例如已接受 ADR 正文里的 "Phase 10"）在**明确属于历史叙述**时可以保留，
   但**当前 / 未来的规范性引用必须使用新编号**。
5. **本记录不启动任何 Phase。** Phase 8 保持 **Ready / Not Started**，
