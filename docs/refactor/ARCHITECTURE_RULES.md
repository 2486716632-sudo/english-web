...
**日期:** 2026-07-29 (修正版)
**状态:** 生效（架构规则**本体**）；2026-09-20 的 future phase 引用更新与 APP-004
portfolio-investigation 政策澄清已于 2026-09-20 经外部评审 Approved（Blocking Issues: None）
并生效（见下方变更范围说明）
**适用范围:** 所有 Phase 2–15 的代码实现（Phase 7 及以后的**实现阶段**由 2026-09-16 路线重新
基线确定；2026-09-20 第二次路线修订已于 2026-09-20 经外部评审 Approved 并生效，
见 `MASTER_PLAN.md`、`DECISIONS.md` ADR-015 / ADR-016 / ADR-017 / ADR-018（后两者 `Accepted`）
与 `PORTFOLIO_ENGINEERING_CRITERIA.md`（`生效 / Active`））。

**本次同步的变更范围（R-07 澄清）:**

1. **更新 future phase 引用**（路线图归属，不是架构规则本体）；
2. **澄清 APP-004 的 portfolio-investigation 语义**——由 ADR-018 区分
   **Agent 准入** / **强制工程调查** / **条件性生产采纳**三者，并说明 RAG / 语义与向量检索 /
   MCP / 外部服务边界按各自理由独立评估，既不是 Agent 强制项，也不是**生产**强制交付物。

**依赖 / 分层架构本体未变**——四层结构、源码依赖方向、Port/Adapter、Composition Root、
Domain 纯计算原则均**未重新设计**。本文件只更新适用范围与上述 APP-004 政策措辞。
...
### 规则 APP-004：Agent 准入（阶段无关的不变式）

Agent 模式（LLM 驱动的动态流程）在本架构中**预留但不默认启用**。

**本规则只拥有不变式，不拥有实现阶段。** 具体在哪个 Phase 实现由 `MASTER_PLAN.md` /
`PHASE_STATUS.md` / 当前已批准的任务书拥有，**不得**把阶段编号硬编码进本规则。

1. **本条门槛只适用于 Agent 能力**：Agent **只能**在**已明确批准的 Phase / 任务书**范围内实现。
2. **Agent 的准入条件 = 真实的模型驱动动态决策**（路径 / 工具 / 动作选择无法预先确定）；
   确定性执行继续 **Workflow 优先**（见 ADR-002、APP-002、APP-003）。
3. **当前没有任何网站级 / master Learning Path Agent 被批准**（见 ADR-015）。
4. **RAG / 语义检索 / 向量检索 / MCP / 外部服务边界不适用本条门槛**，因为它们**不要求** Agent 行为。
   它们各自按自身的产品 / 工程理由**独立评估**，且均**不是**路线图的**强制生产交付物**
   （它们可以是**强制工程调查**；生产采纳由证据决定，见 `PORTFOLIO_ENGINEERING_CRITERIA.md`
   与 ADR-018）：
   - 语义检索 / RAG：存在**可证明的检索问题**、且相对更简单的上下文 / 数据访问有**可评估收益**时才引入；
   - 向量检索 / 向量库：语义检索需求**确实优于**更简单的结构化 / 数据库 / 搜索方案时才引入；
   - MCP：存在**真实的互操作 / 可复用工具 / 服务边界**时才引入；
   - 外部服务边界：部署 / 归属 / 扩展 / 复用 / 隔离 / 互操作需求**支持拆分**时才引入。

   **确定性 Workflow + 检索是合法形态**，不需要 Agent。总原则见 `MASTER_PLAN.md` 核心原则 6
   （架构复杂度只为真实的产品 / 工程需要引入）。本条不重述这些技术各自的实现约束。

Agent 相关代码必须放在 `src/application/agents/` 目录。

> **规则来源记录。** 本条当前措辞是**外部评审要求的 reviewer form**（2026-09-16 复审 R-03；
> R-05 进一步把 RAG / 语义与向量检索 / MCP / 外部服务边界从 Agent 准入门槛中**解耦**），
> 依据本文件附录的"工程师 + 审核者同意"流程记录在案。修订只把**实现阶段**与**与 Agent 无关的技术**
> 从本条规则中移出，不扩大、不重写既有架构规则；Workflow / Agent 分离原则保持有效。
> **2026-09-20 补充：** 第二次路线修订（ADR-018）为这些技术引入**可测量的工程调查**门槛
> （实验义务 ≠ 生产义务）；本条只区分"准入是否需要 Agent 行为"，不规定实现阶段。
